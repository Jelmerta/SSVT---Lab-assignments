...
suite('numbers', function () {
  ...
  test('should be able to check equality of two floating point numbers', function (done) {
    ...
    // Additional test cases, doing this with integers because it is difficult
    //to make two divided numbers exactly equal to epsilon when using float division.
    assert.equal(basic.numbersEqual(5, 4, 1), false);
    assert.equal(basic.numbersEqual(5, 6, 1), false);
    done();
  });
  ...
  test('numbersEqual should throw an exception when given a non number', function (done) {
    assert.throws(
      function () {
        basic.numbersEqual(1, 'error', 0.2);
      },
      /Error: First and Second must be numbers/
    );
    done();
  });
  ...
  test('numbersEqual should throw an exception when given a non number', function (done) {
    assert.throws(
      function () {
        basic.numbersEqual('error', 1, 0.2);
      },
      /Error: First and Second must be numbers/
    );
    done();
